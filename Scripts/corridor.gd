...
func build_corridor():
	...
	# --- FLOOR ---
	_make_box(Vector3(0, -0.1, -CORRIDOR_LENGTH / 2),
		Vector3(CORRIDOR_WIDTH, 0.2, CORRIDOR_LENGTH), floor_mat, "CorridorFloor")

	# --- CEILING ---
	_make_box(Vector3(0, CORRIDOR_HEIGHT + 0.1, -CORRIDOR_LENGTH / 2),
		Vector3(CORRIDOR_WIDTH, 0.2, CORRIDOR_LENGTH), ceiling_mat, "CorridorCeiling")

	# --- LEFT WALL — segmented with gaps at z=-8 and z=-20 ---
	_make_box(Vector3(-CORRIDOR_WIDTH/2-0.1, CORRIDOR_HEIGHT/2, -3.5),
		Vector3(0.2, CORRIDOR_HEIGHT, 7.0), wall_mat, "WallLeft1")
	_make_box(Vector3(-CORRIDOR_WIDTH/2-0.1, CORRIDOR_HEIGHT/2, -14.0),
		Vector3(0.2, CORRIDOR_HEIGHT, 6.0), wall_mat, "WallLeft2")
	_make_box(Vector3(-CORRIDOR_WIDTH/2-0.1, CORRIDOR_HEIGHT/2, -26.0),
		Vector3(0.2, CORRIDOR_HEIGHT, 8.0), wall_mat, "WallLeft3")

	# --- RIGHT WALL — segmented with gaps at z=-8 and z=-20 ---
	_make_box(Vector3(CORRIDOR_WIDTH/2+0.1, CORRIDOR_HEIGHT/2, -3.5),
		Vector3(0.2, CORRIDOR_HEIGHT, 7.0), wall_mat, "WallRight1")
	_make_box(Vector3(CORRIDOR_WIDTH/2+0.1, CORRIDOR_HEIGHT/2, -14.0),
		Vector3(0.2, CORRIDOR_HEIGHT, 6.0), wall_mat, "WallRight2")
	_make_box(Vector3(CORRIDOR_WIDTH/2+0.1, CORRIDOR_HEIGHT/2, -26.0),
		Vector3(0.2, CORRIDOR_HEIGHT, 8.0), wall_mat, "WallRight3")

	# --- BACK WALL ---
	_make_box(Vector3(0, CORRIDOR_HEIGHT / 2, 0.1),
		Vector3(CORRIDOR_WIDTH, CORRIDOR_HEIGHT, 0.2), wall_mat, "WallBack")
	...
	for side in sides:
		for i in range(NUM_SIDE_ROOMS):
			...
			_build_side_room(Vector3(x_offset, 0, z), side, room_name,
				wall_mat, floor_mat, ceiling_mat, door_mat,
				metal_mat, glass_mat, paper_mat, screen_mat, blood_mat)

	# --- BRIDGE FLOOR to end room ---
	_make_box(Vector3(0, -0.1, -CORRIDOR_LENGTH - 1.0),
		Vector3(CORRIDOR_WIDTH, 0.2, 2.2), floor_mat, "BridgeFloor")

	# --- END ROOM ---
	_build_end_room(Vector3(0, 0, -CORRIDOR_LENGTH - ROOM_SIZE),
		wall_mat, floor_mat, ceiling_mat, metal_mat, glass_mat, screen_mat, blood_mat)
...
func _build_side_room(center: Vector3, side: int, room_name: String,
	...
	# Front wall — wide open doorway (4 units gap)
	_make_box(Vector3(x - side * (ROOM_DEPTH / 2 - 0.1), CORRIDOR_HEIGHT / 2, z - ROOM_SIZE / 2 + 0.7),
		Vector3(0.2, CORRIDOR_HEIGHT, ROOM_SIZE - 4.6), wall_mat, room_name + "_FrontWallA")
	_make_box(Vector3(x - side * (ROOM_DEPTH / 2 - 0.1), CORRIDOR_HEIGHT / 2, z + ROOM_SIZE / 2 - 0.7),
		Vector3(0.2, CORRIDOR_HEIGHT, ROOM_SIZE - 4.6), wall_mat, room_name + "_FrontWallB")
	_make_box(Vector3(x - side * (ROOM_DEPTH / 2 - 0.1), CORRIDOR_HEIGHT - 0.4, z),
		Vector3(0.2, 0.7, 4.0), wall_mat, room_name + "_Lintel")

	# Door — swung wide open
	var door = _make_box(
		Vector3(x - side * (ROOM_DEPTH / 2 - 0.25), 1.1, z - 1.5),
...
func _furnish_sample_storage(x, z, side, metal_mat, glass_mat, paper_mat):
	for shelf in range(3):
		...
		for v in range(5):
			_make_box(Vector3(x + side * 2.6, 0.65 + shelf * 0.7, z - 1.5 + v * 0.7),
...
func _build_end_room(center: Vector3, wall_mat, floor_mat, ceiling_mat,
		metal_mat, glass_mat, screen_mat, blood_mat):
	var x = center.x
	var z = center.z
